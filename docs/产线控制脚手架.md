...
- **合成配方**（`CraftingId: moonterminal` 已写在 csv，未见 `.ier`）
- **通电自动运行**（Boot / startup）
- **`terminal.storage`** 等轻量控制状态存盘
...
### P0 — 可玩闭环（进入产线 API 前建议完成）

| # | 项 | 说明 |
| --- | --- | --- |
| 1 | **合成配方** | `moonterminal` 对应 `.ier` 或等价配方，生存可造 |
| 2 | **AutoRunOnPower** | Component 字段 + Load/Save；有电且策略开启时自动 `StartScript()`；掉电仍 `StopScript` |
| 3 | **terminal.storage（MVP）** | 字符串或 JSON 字典；Lua `get`/`set`/`clear`；Save/Load 对称 |
...
6. **Screen 几何** — 改 `Screen` / Provider 绘制时遵循主仓 `screen-geometry-anti-patterns.mdc`；纹理用 `Screen.QuadRasterizerState`，3D 文字用 `Screen.FontRasterizerState`。  
...
| 路径 | 说明 |
| --- | --- |
| `Components/ComponentMoonTerminal.cs` | 脚本与输出；待加 AutoRun / storage |
